...
type TotlichenLinieConstructor = IGConstructor<TotlichenLinie>;

const TotlichenLinie : TotlichenLinieConstructor = ig.GuiElementBase.extend({
	pattern: null,
	settings: {
		gradientSize: 80,
		gradientTimer: 1,
	...
	init: function() {
		...
		this.settings.gradientScale = this.settings.gradientSize / 128;
		this.timer = 0;
	...
	updateDrawablesAnfang: function(g : ig.GuiRenderer) {
		...
		if (oben > 0) {
			g.addColor("black", 0, 0, ig.system.width, oben);
			g.addColor("black", 0, ig.system.height - oben,
				   ig.system.width, oben);
		}
		const alphaOben = Math.max(oben, 0);
		g.addColor(`rgb(255,255,255,${quotient})`, 0, alphaOben,
			   ig.system.width, ig.system.height - 2 * alphaOben);
		...
		g.addTransform().setTranslate(0, ig.system.height - unten)
				.setScale(1, massstab);
		...
		g.addTransform().setTranslate(0, unten).setScale(1, -massstab);
		g.addPattern(this.pattern,
			     0, 0,
			     0, vonY,
			     ig.system.width, quellhohe);
...
/*
	...
	updateDrawablesEnde: function(g: ig.GuiRenderer) {
		...
		trafo.setPivot(ig.system.width / 2, ig.system.height / 2)
		...
		const weite = ig.system.width * settings.lineInitialScale;
		const links = (weite - ig.system.width) / -2;
		const oben = (ig.system.height - settings.lineSize) / 2;

		g.addColor(cssfarbe, links, oben, weite, settings.lineSize);
		g.undoTransform();
	},
...
export { TotlichenLinie, TotlichenLinieConstructor };
